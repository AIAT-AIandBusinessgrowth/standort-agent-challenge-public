# AI Developer Challenge: „Standort-Agent"

Version 1.0 · 2026-06 · Kontakt: AI:AT Hiring Team

> Synthetisch / vereinfacht, Stand 2026-06, keine offizielle AI:AT-Position.

Schön, dass du dabei bist. Das hier ist kein Trick-Test. Es ist ein kleines, echtes Stück von dem, was wir im Venture Studio & n8n CoE täglich tun: mit KI schnell etwas Lauffähiges bauen, das ein echtes Entscheidungsproblem zerlegt. Diesmal etwas ambitionierter, nämlich ein Multi-Agent-System.

Es gibt keine Musterlösung, die du treffen musst. Wir schauen, wie du ein unscharf spezifiziertes Problem in Teilaufgaben zerlegst, eine Architektur entwirfst, Entscheidungen triffst und KI als Hebel einsetzt, und wie du sie prüfst.
...
Der Clou: Standortbewertung ist mehrdimensional. Demografie, POI-/Umfeld-Dichte, Mietkosten und ÖV-Anbindung ziehen in unterschiedliche Richtungen. Genau das macht es zu einer schönen Multi-Agent-Aufgabe: pro Signal ein spezialisierter Subagent, ein Orchestrator, der aggregiert und gewichtet.

Wir liefern dir ein Seed-Dataset mit 20 Gemeinden (`municipalities.json` + `municipalities.csv`, Felder dokumentiert) und 2–3 Beispiel-Profile. Kein Domänenwissen nötig: alles, was du brauchst, steckt im Dataset.

> 📂 Im Repo: [`../data/`](../data/) mit `municipalities.json`, `municipalities.csv` und Beispiel-Profilen unter [`../data/examples/`](../data/examples/) (Feld-Doku: [`../data/README.md`](../data/README.md)).
...
- **Verarbeitung:** vier Signale, nämlich Demografie (Altersstruktur ↔ Zielgruppe), POI/Umfeld (POI-Dichte ↔ Branche), Miete (Mietindex ↔ Budget) und ÖV (Anbindung ↔ Zielgruppe). Jedes Signal wird zu einem Teil-Score; der Orchestrator aggregiert sie zu einem gewichteten Gesamt-Ranking.
- **Output:** ein begründetes Ranking der passendsten Gemeinden, z. B. die Top 3–5 (die Zahl ist deine Entscheidung, begründe sie kurz), je „passt, weil… / passt nicht, weil…" pro Signal, plus ein interaktiver HTML-Report mit Ranking-Tabelle und Begründungen.
- **Form:** lauffähiges Git-Repo mit echtem Code und ein README, das uns das Ding in < 5 min zum Laufen bringt. Stack: Python (≥ 3.11). Welche Libraries, ob CLI oder mehr, ist dir überlassen.
...
> Echte LLM-Calls (z. B. für die generierten Begründungen) sind erlaubt, aber nicht Pflicht: eine deterministisch oder templatebasiert erzeugte Begründung ist völlig okay. Kein API-Key nötig, falls du keinen hast, denn der Basis-Lauf muss ohne Schlüssel funktionieren.

### Teil B: Code-Review (~15 min)

Wir geben dir ein kurzes, KI-generiertes Code-Snippet (Python). Tipp: vergleiche, was der Doc-Kommentar verspricht, mit dem, was der Code tut, und denk an den Hinweis zu Skalen und Richtung oben. Es gibt einen zentralen funktionalen Bug: finde ihn, fixe ihn, und begründe in 3–5 Sätzen, warum es einer ist. Sag uns auch, was dir sonst auffällt (fehlende Edge-Cases, fragwürdige Annahmen, fehlende Validierung). Diese weiterführende Kritik zählt für uns genauso wie der Fix selbst. Wir wollen sehen, ob du KI-Output beurteilen kannst, nicht nur erzeugen.

> Das Snippet liegt als Datei unter [`code-review/snippet.py`](./code-review/snippet.py) und ist unten in Anhang A abgedruckt.

## Spielregeln

- **Aufwand: ~4–6 fokussierte Stunden.** Du hast eine Woche, die ist für Flexibilität da, nicht zum Durchgrinden. Bitte nicht überinvestieren: wir bewerten Denken und Urteil, nicht Politur. *(Die 4–6 h schließen Walkthrough + Decision-Log ein. Loom: 1 Take, 3–5 min; Decision-Log: 5–10 Zeilen Stichworte.)*
- **Nutze jede KI, jede Library, google frei.** Wird erwartet, nicht nur erlaubt. Cursor, Claude, Copilot, Coding-Agents: leg los.
- **Der Kern (A + B) ist die Latte.** Dazu gehören auch ein paar sinnvolle Tests (kein Coverage-Theater; zählen mit 10 %). Echte Stretch-Goals (API-Integration via OSM Overpass, UI, Eval-Set, Deployment, LLM-generierte Begründungen, Konfidenzintervalle) sind zum Glänzen da, komplett optional.
...
4. **Kurzes Decision-Log + Schlüssel-Prompts:** 5–10 Zeilen Entscheidungen und Trade-offs (Subagent-Schnitt, Gewichtung, Umgang mit den unterschiedlichen Signal-Skalen) plus die KI-Prompts, die den Unterschied gemacht haben. Zeig uns, wie du mit KI zusammenarbeitest. Das ist genau die Fähigkeit, für die wir die Rolle besetzen.
...
Kurzer **Live-Walkthrough (30–45 min)**: du zeigst dein Ergebnis, wir setzen live eine neue Anforderung drauf, z. B. ein fünftes Signal bzw. einen neuen Subagent hinzufügen, und schauen, wie du deine eigene Architektur erweiterst. Die neue Anforderung ist bewusst klein. Es geht darum, wie du laut denkst, nicht um ein perfektes Ergebnis in 10 Minuten. Deine gewohnten KI-Tools darfst du dabei nutzen, genau wie beim Bauen. Danach zeigen wir dir unsere eigene Lösung und reden ehrlich darüber. Jede:r bekommt Feedback, egal wie's ausgeht.

> **Fair & transparent.** Diese Challenge ist unbezahlt. Dafür zeigen wir dir nach dem Debrief unsere eigene Lösung, mit echten Entscheidungen, Prompts und Trade-offs. Das ist unser „Learn"-Versprechen in Aktion. Und: strukturiertes, ehrliches Feedback an jede:n, egal wie der Prozess ausgeht. Kein Ghosting. Nie.

Wir sind gespannt, wie du mit KI ein Multi-Agent-System baust, das eine €500K-Entscheidung nachvollziehbar macht.
